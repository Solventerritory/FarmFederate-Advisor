# ================== Colab One-Cell Multimodal Farm Advisor ==================
# This cell:
#  - Installs deps
#  - Defines full multimodal federated LoRA trainer (text + images)
#  - Builds LocalMini + HF text + HF image datasets (best-effort)
#  - Runs training with configurable ArgsOverride at the bottom
# ============================================================================

# pip install "transformers>=4.40" datasets peft torchvision scikit-learn  # Run this in terminal first
...
# --------------------- Optional: set HF token here --------------------------
# If you have a Hugging Face token, uncomment and paste:
...
def get_args():
    ...
    ap.add_argument(
        ...
        type=str,
        default="images",
        help="root dir for images (download + local)",
    )
    ap.add_argument(
        "--image_csv",
        type=str,
        default="",
        help="CSV with columns filename,text,labels (user-provided)",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.90,
        help="per-class precision target during calibration",
    )
    ap.add_argument(
        "--dirichlet_alpha",
        type=float,
        default=0.25,
        help="smaller => stronger non-IID",
    )
    ap.add_argument(
        "--participation",
        type=float,
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.05,
        help="probability a sampled client drops",
    )
    ap.add_argument(
        "--prior_scale",
        type=float,
        default=0.30,
        help="scales sensor priors effect (0..1)",
    )
    ap.add_argument(
        "--label_noise",
        type=float,
        default=0.05,
        help="flip/remove labels with this prob",
    )
    ...
    ap.add_argument(
        "--offline",
        action="store_true",
        help="use local cache only (transformers/datasets)",
    )
...
KW = {
    "water": [
        "dry","wilting","wilt","parched","drought","moisture","irrigation","canopy stress",
        "water stress","droop","cracking soil","hard crust","soil moisture low",
    ],
    "nutrient": [
        "nitrogen","phosphorus","potassium","npk","fertilizer","fertiliser","chlorosis",
        "chlorotic","interveinal","leaf color chart","lcc","spad","low spad",
        "older leaves yellowing","necrotic margin","micronutrient","deficiency",
    ],
    "pest": [
        "pest","aphid","whitefly","borer","hopper","weevil","caterpillar","larvae","thrips",
        "mites","trap","sticky residue","honeydew","chewed","webbing","frass","insect",
    ],
    "disease": [
        "blight","rust","mildew","smut","rot","leaf spot","necrosis","pathogen","fungal",
        "bacterial","viral","lesion","mosaic","wilt disease","canker","powdery mildew","downy",
    ],
    "heat": [
        "heatwave","hot","scorch","sunburn","thermal stress","high temperature","blistering",
        "desiccation","sun scorch","leaf burn","heat stress",
    ],
}
...
def weak_labels(text: str) -> List[int]:
    ...
    if any(k in t for k in KW["nutrient"]):
        strong_n = any(
            x in t
            for x in [
                "chlorosis","chlorotic","interveinal","npk","nitrogen","potassium",
                "leaf color chart","lcc","low spad","spad",
            ]
        )
        qualified_yellow = "yellowing" in t and (
            "older leaves" in t or "old leaves" in t
        )
...
def _load_ds(name, split=None, streaming=False):
    if not HAS_DATASETS:
        raise RuntimeError(
            "datasets lib not available; install via `pip install datasets`"
        )
    ...
    for attempt in range(4):
        try:
            ...
            if any(
                x in str(e)
                for x in [
                    "429","Read timed out","504","Temporary failure","Connection",
                ]
            ):
...
def prepare_images_from_hf(
    dataset_name: str, max_items: int, image_dir: str
) -> pd.DataFrame:
    rows = []
    if not HAS_DATASETS:
        ...
        try:
            if isinstance(img_val, Image.Image):
                local_fname = os.path.join(
                    image_dir, f"{dataset_name.replace('/', '_')}_{cnt}.jpg"
                )
                ...
                if local_fname is None:
                    url = (
                        img_val.get("url") or img_val.get("img_url") or img_val.get("image_url")
                    )
                    if url:
                        local_fname = os.path.join(
                            image_dir, f"{dataset_name.replace('/', '_')}_{cnt}.jpg"
                ...
                if img_val.startswith("http"):
                    ...
                    if os.path.exists(img_val):
                        local_fname = os.path.join(
                            image_dir, os.path.basename(img_val)
                        )
                        shutil.copyfile(img_val, local_fname)
            elif (
                isinstance(img_val, list)
                and len(img_val) > 0
                and isinstance(img_val[0], str)
                and img_val[0].startswith("http")
            ):
                url = img_val[0]
                local_fname = os.path.join(
                    image_dir, f"{dataset_name.replace('/', '_')}_{cnt}.jpg"
...
# --------------------- Image CSV builder ---------------------
def build_corpus_with_images(
    image_csv: str, image_root: str = "", max_samples: int = 0
) -> pd.DataFrame:
...
def oversample_by_class(
    df: pd.DataFrame, target_each_map: Dict[int, int] = None
) -> pd.DataFrame:
...
def summarize_labels(df: pd.DataFrame, tag="set"):
    ...
    print(
        f"[{tag}] label counts:",
        {ISSUE_LABELS[i]: int(c) for i, c in enumerate(counts)},
    )
...
def build_corpus() -> pd.DataFrame:
    ...
    - text_df: LocalMini + optional HF text datasets (gardian / argilla / agnews)
    - img_df: HF image datasets (PlantVillage, PlantDoc, Cassava, etc.) or user CSV

    If ARGS.use_images:
        final_df = concat(text_df (with empty image_path) + img_df)
    Else:
        final_df = text_df only.
    """
    # ----- TEXT PART -----
    if ARGS.dataset == "mix":
        ...
        text_df = build_localmini(
            ARGS.max_samples or 0, ARGS.mqtt_csv, ARGS.extra_csv
        )
    else:
        if ARGS.dataset == "gardian" and HAS_DATASETS:
            raws = build_gardian_stream(ARGS.max_per_source)
            rows = [
                (fuse_text(simulate_sensor_summary(), r), weak_labels(r))
                for r in raws
            ]
            text_df = pd.DataFrame(
                [(t, l) for (t, l) in rows if l], columns=["text", "labels"]
            )
    ...
    if len(text_df) == 0:
        raise RuntimeError(
            "Empty dataset after filtering. Include localmini or lower caps."
        )
    ...
    if ARGS.use_images and ARGS.image_csv and os.path.exists(ARGS.image_csv):
        ...
        img_df = build_corpus_with_images(
            ARGS.image_csv, ARGS.image_dir, max_samples=ARGS.max_samples
        )

    # Case 2: HF image datasets
    elif ARGS.use_images:
        print("[Build] Harvesting HF image datasets (PlantVillage / PlantDoc / Cassava / etc.)")
        parts = []
        ...
        if parts:
            ...
            print("[Build] No HF image data harvested.")
        ...
        for c in needed_cols:
            if c not in img_df.columns:
                if c == "image_path":
                    ...
                    raise RuntimeError(f"img_df missing required column: {c}")
        final_df = pd.concat(
            [text_df[needed_cols], img_df[needed_cols]],
            ignore_index=True,
            sort=False,
        )
        final_df = final_df.sample(frac=1.0, random_state=SEED).reset_index(drop=True)
        print(f"[Build] final multimodal size (text+images): {len(final_df)}")
...
def infer_lora_targets_from_model(model) -> List[str]:
    ...
    for cands in cand_sets:
        found = [
            c for c in cands if any(("." + c) in n or n.endswith(c) for n in names)
        ]
        if len(found) >= 2:
            return found
    return ["classifier"]
...
def build_text_model(num_labels: int, freeze_base: bool = True):
    ...
    try:
        ...
        print(f"[Warn] failed to load pretrained weights for {ARGS.model_name}: {e}")
        print("[Warn] Falling back to random-init from local config (no pretrained weights).")
...
class MultiModalModel(nn.Module):
    ...
    def __init__(
        ...
        try:
            text_base = AutoModel.from_pretrained(
                text_model_name, local_files_only=ARGS.offline
            )
        except OSError as e:
        ...
        if not HAS_PEFT:
            raise RuntimeError(
                "peft not available: install `pip install peft` to use LoRA"
            )
        ...
        # Vision encoder  (ROBUST OFFLINE / RATE-LIMIT SAFE)
        try:
            self.vision = ViTModel.from_pretrained(
                vit_name, local_files_only=ARGS.offline
            )
        except Exception as e:
            print(f"[Warn] failed to load pretrained ViT {vit_name}: {e}")
            print("[Warn] Falling back to randomly initialized ViT.")
            # Try loading its config locally; if that also fails, build a minimal ViTConfig.
            try:
                vcfg = AutoConfig.from_pretrained(
                    vit_name, local_files_only=True
                )
            except Exception as e2:
        ...
        print(
            f"[Model] text_dim={text_dim} vision_dim={vision_dim} lora_targets={targets}"
        )
    ...
    def forward(self, input_ids=None, attention_mask=None, image=None):
        ...
        if image is None:
            ...
            if (
                hasattr(vit_out, "pooler_output")
                and vit_out.pooler_output is not None
            ):
...
def evaluate_with_thr(model, loader, thr) -> Dict[str, float]:
    ...
    if not ARGS.quiet_eval:
        ...
        for i, lab in enumerate(ISSUE_LABELS):
            if supports[i] < 20:
                print(
                    f" - {lab:14s} | insufficient support (n={int(supports[i])})"
                )
                continue
            print(
                f" - {lab:14s} | P={_cap(prec[i]):.3f} R={_cap(rec[i]):.3f} "
                f"F1={_cap(f1s[i]):.3f} thr={thr[i]:.2f}"
            )
        print(
        ...
        try:
            ...
            print(
                f"AUPRC micro={_cap(pr_micro):.3f} macro={_cap(pr_macro):.3f} | "
...
def save_tables_and_plots(metrics: Dict[str, float], save_dir: str):
    ...
    df = pd.DataFrame(
        {"Label": ISSUE_LABELS, "Precision": prec, "Recall": rec, "F1": f1s}
    )
...
def train_local(
    model, tok, tr_df, va_df, class_alpha: torch.Tensor
    ...
    loss_fn = FocalLoss(
        alpha=class_alpha.to(DEVICE), gamma=2.5, label_smoothing=0.02
    )
    model.train()
    model.to(DEVICE)
    ...
    thr = calibrate_thresholds(
        model, va_loader, precision_target=ARGS.precision_target
    )
    ...
    try:
        lora_sd = get_peft_model_state_dict(
            model if not hasattr(model, "text_encoder") else model.text_encoder
        )
    except Exception:
...
def fedavg_weighted(states: List[Dict[str, torch.Tensor]], sizes: List[int]) -> Dict[str, torch.Tensor]:
    ...
    for k in keys:
        out[k] = torch.stack(
            [st[k].float() * w for st, w in zip(states, ws)], dim=0
        ).sum(0)
    return out
...
@torch.no_grad()
def mc_predict_probs(
    model, tok, texts: list, images: Optional[List[torch.Tensor]] = None, T: int = 8
    ...
    img_tensor = None
    if ARGS.use_images and images is not None and len(images) > 0:
...
def build_rationales(text: str, mean_probs: np.ndarray, std_probs: np.ndarray, thr: np.ndarray):
    ...
    for i, lab in enumerate(ISSUE_LABELS):
        ...
        if sensors:
            ...
            if lab == "water_stress" and (
                (sm is not None and sm <= 20) or (vpd is not None and vpd >= 2.0)
            ):
                reasons.append("sensor: low soil moisture / high VPD")
            if lab == "heat_stress" and (
                (t_air is not None and t_air >= 36)
                or (vpd is not None and vpd >= 2.2)
            ):
                reasons.append("sensor: high temperature / VPD")
            if lab == "disease_risk" and (
                (h is not None and h >= 70)
                or (sensors.get("rf", 0) >= 2.0)
            ):
            ...
            if lab == "nutrient_def" and (sensors.get("ph") is not None) and (
                sensors["ph"] < 5.8 or sensors["ph"] > 7.4
            ):
...
def pretty_multihypo_output(text: str, ranked: dict, rationales: dict):
    ...
    if ranked["active"]:
        ...
        for lab, p, m in ranked["active"]:
            out.append(
                f"   - {lab}: p={p:.2f}  Δ={m:+.02f}  ⇒ {ADVICE[lab]}"
            )
            if rationales.get(lab):
                out.append(f"     · why: {rationales[lab]}")
    else:
    ...
    if ranked["near"]:
        ...
        for lab, p, m in ranked["near"]:
            out.append(
                f"   - {lab}: p={p:.2f}  Δ={m:+.02f}  (monitor / consider preventive steps)"
            )
            if rationales.get(lab):
                out.append(f"     · why: {rationales[lab]}")
    if ranked["top"]:
...
def predict(
    model, tok, text: str, thr: np.ndarray, image_tensor: Optional[torch.Tensor] = None
) -> List[int]:
    model.eval()
    model.to(DEVICE)
    enc = tok(
...
def run_training():
    if not HAS_PEFT and ARGS.use_images:
        print(
            "[Warn] peft not available. Install `pip install peft` before attempting LoRA training."
        )
    print(
        f"Device: {DEVICE} (AMP={'on' if amp_enabled() else 'off'})  "
        f"model={ARGS.model_name}  use_images={ARGS.use_images}"
    )
    tok = build_tokenizer()
    df = build_corpus()
    ...
    alpha[1] *= 1.2  # small boost for nutrient_def
    ...
    if multimodal:
        ...
        global_model = build_text_model(NUM_LABELS, freeze_base=ARGS.freeze_base).to(
            DEVICE
        )
    ...
    if "image_path" in val_df.columns and ARGS.use_images:
        ...
        val_loader = DataLoader(
            MultiLabelDS(val_df, tok, ARGS.max_len),
            batch_size=32,
            shuffle=False,
        )
    ...
    def evaluate_global(thr):
        if "image_path" in test_df.columns and ARGS.use_images:
            ...
            test_loader = DataLoader(
                MultiLabelDS(test_df, tok, ARGS.max_len),
                batch_size=64,
                shuffle=False,
            )
    ...
    for r in range(1, max(1, ARGS.rounds) + 1):
        ...
        for i in chosen:
            ...
            if multimodal:
                ...
                try:
                    set_peft_model_state_dict(
                        local.text_encoder,
                        get_peft_model_state_dict(global_model.text_encoder),
                    )
                except Exception:
                    try:
                        set_peft_model_state_dict(
                            local, get_peft_model_state_dict(global_model)
                        )
                    except Exception:
                        pass
            else:
                local = build_text_model(NUM_LABELS, freeze_base=ARGS.freeze_base).to(
                    DEVICE
                )
                try:
                    set_peft_model_state_dict(
            ...
            local_epochs = int(
                rng_local.choice([2, 3], p=[0.6, 0.4])
            )
            orig_local = ARGS.local_epochs
            ARGS.local_epochs = local_epochs

            micro, macro, lora_sd, thr_local, used_n = train_local(
                local, tok, tr_df, va_df, class_alpha=alpha
            )
            ARGS.local_epochs = orig_local

            print(
                f"[Client {i+1}] micro_f1={_fmt_str(micro)} "
                f"macro_f1={_fmt_str(macro)} (n={len(cdf)}) "
                f"thr={np.round(thr_local,2)}"
            )
        ...
        if states:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        "[Warn] couldn't set averaged peft state directly; skipping this round."
                    )
            ...
            final_thr = np.clip(
                final_thr + np.array([+0.03, 0.00, 0.00, +0.02, 0.00]),
                0.05,
                0.90,
            )
        else:
        ...
        np.save(
            os.path.join(metrics_dir, f"{round_tag}_thr.npy"),
            final_thr,
        )
        with open(
            os.path.join(metrics_dir, f"{round_tag}_summary.json"), "w"
        ) as f:
            json.dump(
                {
                    "round": r,
    ...
    np.save(
        thp,
        thr_history[-1] if thr_history else np.array([0.5] * NUM_LABELS),
    )
    ...
    if "image_path" in test_df.columns and ARGS.use_images:
        ...
        cal_df = test_df.sample(min(400, len(test_df)), random_state=SEED)
        cal_loader = DataLoader(
            MultiLabelDS(cal_df, tok, ARGS.max_len),
            batch_size=32,
            shuffle=False,
        )
    mets = evaluate_with_thr(
        global_model,
        cal_loader,
        thr_history[-1] if thr_history else np.array([0.5] * NUM_LABELS),
    )
    ...
    for s in cal_df.sample(min(3, len(cal_df)), random_state=SEED)[
        "text"
    ].tolist():
        pred = predict(
            global_model,
...
def run_inference():
    ...
    if multimodal:
        ...
        model = build_text_model(NUM_LABELS, freeze_base=ARGS.freeze_base).to(
            DEVICE
        )
    ...
    if os.path.exists(ap):
        try:
            if multimodal:
                set_peft_model_state_dict(
                    model.text_encoder,
                    torch.load(ap, map_location="cpu"),
                )
            else:
                set_peft_model_state_dict(
                    model, torch.load(ap, map_location="cpu")
                )
            print(f"[Load] adapters {ap}")
        except Exception:
            try:
                model.load_state_dict(
                    torch.load(ap, map_location="cpu"), strict=False
                )
                print(f"[Load] model state {ap}")
            except Exception:
                print(
                    "[Warn] couldn't load adapters; proceeding with random adapters."
                )
    else:
    ...
    sensors = (
        f"SENSORS: {', '.join([x.strip() for x in ARGS.sensors.split(',')])}."
        if ARGS.sensors.strip()
        else simulate_sensor_summary()
    )
    q = (
        ARGS.query.strip()
        or "Slight leaf curling after hot afternoons; few insects seen under leaves."
    )
    text = f"{sensors}\nLOG: {q}"

    mean_probs, std_probs = mc_predict_probs(
        model, tok, [text], T=max(1, ARGS.samples)
    )
...
# --------------------- Main (Colab: runs training by default) -------------
if ARGS.inference:
    run_inference()
else:
    run_training()
