...
class FocalLoss(nn.Module):
    """
    Multi-label focal loss with optional per-class alpha weights.

    Robust to:
    - logits being a Tensor OR HF-style object/dict with `.logits`
    - targets being a Tensor OR object/dict with `.labels` / `.y` / `.targets`
    """
    def __init__(
        self,
    ...
    def _extract_logits(self, logits):
        """
        Normalize logits into a torch.Tensor of shape [B, C].
        """
        # Tensor directly
        ...
        if isinstance(logits, dict):
            for key in ("logits", "preds", "output"):
                if key in logits and isinstance(logits[key], torch.Tensor):
                    return logits[key]

        # HF outputs / SimpleNamespace, etc.
        for key in ("logits", "preds", "output"):
            if hasattr(logits, key):
                t = getattr(logits, key)
                if isinstance(t, torch.Tensor):
                    return t

        raise TypeError(
            f"FocalLoss expected logits as Tensor or an object/dict containing "
            f"'logits' / 'preds' / 'output' Tensor, but got {type(logits)}."
        )

    def _extract_targets(self, targets):
        """
        Normalize targets into a torch.Tensor of shape [B, C].
        """
        # Already tensor
        ...
        for key in ("labels", "y", "targets"):
            if hasattr(targets, key):
                t = getattr(targets, key)
                if isinstance(t, torch.Tensor):
                    return t

        raise TypeError(
            f"FocalLoss expected targets as Tensor or an object/dict containing "
            f"'labels' / 'y' / 'targets' Tensor, but got {type(targets)}."
        )

    def forward(self, logits, targets):
        """
        logits:  Tensor or HF-style output
        targets: Tensor or batch-like object
        """
        logits_t = self._extract_logits(logits)
        targets_t = self._extract_targets(targets).to(logits_t.device).float()

        # Shape check
        if targets_t.shape != logits_t.shape:
            raise ValueError(
                f"FocalLoss: target shape {targets_t.shape} does not match "
                f"logits shape {logits_t.shape}."
            )

        # Label smoothing (keep in [0,1])
        if self.smooth > 0:
            targets_t = targets_t * (1.0 - self.smooth) + 0.5 * self.smooth

        # BCE per example, per class
    ...
    for epoch in range(local_epochs):
        for it, batch in enumerate(train_loader, start=1):
            ...
            with torch.amp.autocast("cuda", enabled=torch.cuda.is_available()):
                ...
                # outputs might be Tensor or HF-style object; FocalLoss handles both
                loss = loss_fn(outputs, labels) / max(1, grad_accum)
