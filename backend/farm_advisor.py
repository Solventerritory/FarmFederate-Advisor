...
# -*- coding: utf-8 -*-
"""
farm_advisor.py — Multimodal (ViT + RoBERTa w/ LoRA) federated multi-label agri issue detector.

Notes:
 - LoRA adapters applied to the text encoder (RoBERTa).
 - Vision encoder uses HuggingFace ViTModel.
 - Federated aggregation only averages the LoRA adapter weights (text side).
 - Use --use_images --image_csv / --image_dir to enable image+text training.
"""
import os, re, math, time, gc, random, argparse, hashlib, json
...
# vision preprocessing (PIL + torchvision transforms)
...
def get_args():
    ...
    ap.add_argument("--dataset", type=str, default="mix",
                    choices=["localmini","gardian","argilla","agnews","mix"])
    ...
    ap.add_argument("--image_dir", type=str, default="", help="root dir for images (optional)")
    ap.add_argument("--image_csv", type=str, default="", help="CSV with columns filename,text,labels (labels comma sep indices or names)")
    ...
    ap.add_argument("--cap_metric_print_at", type=float, default=0.999,
                    help="Cap printed P/R/F1/micro/macro below 1.0 to avoid 1.000 displays")
    ap.add_argument("--quiet_eval", action="store_true", help="reduce per-round metric verbosity")
    ...
    # Inference multi-hypothesis controls
    ap.add_argument("--topk", type=int, default=3, help="Top-k labels to display by probability")
    ap.add_argument("--epsilon", type=float, default=0.07, help="Near-miss band below threshold")
    ap.add_argument("--samples", type=int, default=16, help="MC-Dropout passes for uncertainty")
...
# HF dataset helpers (unchanged from before)
def _load_ds(name, split=None, streaming=False):
    if not HAS_DATASETS: raise RuntimeError("datasets lib not available")
...
# --------------------- Oversampling & noise ---------------------
def oversample_by_class(df: pd.DataFrame,
                        target_each_map: Dict[int,int] = None) -> pd.DataFrame:
...
# --------------------- Build corpus (image CSV support) ---------------------
def build_corpus() -> pd.DataFrame:
    # if user requested image_csv, prefer that as labeled multimodal dataset
    ...
    print(f"[Build] final size: {len(df)}")
...
def build_corpus_with_images(image_csv:str, image_root:str="", max_samples:int=0) -> pd.DataFrame:
    ...
    for _, r in df_raw.iterrows():
        ...
        fname = str(r.get("filename","")).strip() if "filename" in r else ""
...
class MultiModalDS(Dataset):
    ...
    def _load_image(self, path: str):
        if not path: return torch.zeros(3, self.img_size, self.img_size, dtype=torch.float32)
...
# --------------------- Tokenizer + multimodal model + LoRA on text ---------------------
def build_tokenizer():
...
class MultiModalModel(nn.Module):
    ...
    def __init__(self, text_model_name, vit_name, num_labels,
        ...
        super().__init__()
        # build base text encoder (AutoModel) and then apply PEFT
        ...
        # infer LoRA target modules and wrap
        targets = infer_lora_targets(text_base)
        ...
        self.text_encoder = text_peft  # peft model (we'll save adapters from here)
    ...
    def forward(self, input_ids=None, attention_mask=None, image=None):
        ...
        if image is None:
            ...
            # ViT expects pixel_values shaped (B, 3, H, W), already normalized
...
def train_local(model, tok, tr_df, va_df, class_alpha:torch.Tensor) -> Tuple[float,float,Dict,np.ndarray,int]:
        ...
        for it, batch in enumerate(tr_loader, start=1):
            ...
            with torch.amp.autocast("cuda", enabled=amp_enabled()):
                if img is not None:
                    ...
                    out = model(input_ids=text_inputs["input_ids"], attention_mask=text_inputs["attention_mask"], image=None)
    ...
    # extract LoRA state dict for text encoder only
...
def mc_predict_probs(model, tok, texts: list, images: Optional[List[torch.Tensor]]=None, T: int = 16) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if ARGS.use_images and images is not None:
        # images is list of PIL or tensors; ensure stacked tensor
        if isinstance(images[0], torch.Tensor):
            img_tensor = torch.stack(images).to(DEVICE)
        else:
            # assume already preprocessed tensors
            img_tensor = torch.stack(images).to(DEVICE)
    probs_stack = []
...
def run_training():
    print(f"Device: {DEVICE} (AMP={'on' if amp_enabled() else 'off'})  text_model={ARGS.model_name} vit={ARGS.vit_name}")
    tok = build_tokenizer()
    df  = build_corpus()
    ...
    # global multimodal model (text encoder contains LoRA adapters)
    ...
    for r in range(1, max(1, ARGS.rounds)+1):
        ...
        for i in chosen:
            ...
            # build local model - same architecture
            local = MultiModalModel(ARGS.model_name, ARGS.vit_name, NUM_LABELS,
                                    freeze_text=ARGS.freeze_base, freeze_vision=ARGS.freeze_vision,
                                    lora_r=ARGS.lora_r, lora_alpha=ARGS.lora_alpha, lora_dropout=ARGS.lora_dropout).to(DEVICE)

            # set local text peft weights from global adapters
        ...
        if states:
            ...
            # set averaged adapters into global text encoder
    ...
    ap = os.path.join(ARGS.save_dir, "global_lora_text.pt")
    ...
    print(f"[Save] adapters (text) → {ap}")
    print(f"[Save] thresholds → {thp}")

    # Final diagnostics on a slice of test set
...
def run_inference():
    tok   = build_tokenizer()
    # build multimodal model and load adapters
    ...
    sensors = (f"SENSORS: {', '.join([x.strip() for x in ARGS.sensors.split(',')])}."
               if ARGS.sensors.strip() else simulate_sensor_summary())
