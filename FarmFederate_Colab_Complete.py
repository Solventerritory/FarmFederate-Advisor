...
# 5) Optional: If you have pasted the full script into a cell, offer to write it to disk so the helper subprocess can find it
try:
    write_script = input('\nDid you paste the full script into a cell and want to write it to FarmFederate_Colab_Complete.py? (Y/n): ').strip().lower()
    write_script = write_script if write_script != '' else 'y'
    if write_script == 'y':
        try:
            nb_content = get_ipython().user_ns.get('In')[-1]
            with open('FarmFederate_Colab_Complete.py', 'w', encoding='utf-8') as fh:
                fh.write(nb_content)
            print('Wrote FarmFederate_Colab_Complete.py to current working directory.')
        except Exception as e:
...
# 6) Run a dry-run (validation) of the main script
print('\nRunning quick DRY-RUN validation (no downloads) ...')
!python FarmFederate_Colab_Complete.py --dry-run
# 7) Automatically proceed to full run (non-interactive)
...
# If you pasted the full script into a notebook cell, optionally write it to disk so the helper can run it as a subprocess
try:
    write_script = input('\nDid you paste the full script into a cell and want to write it to FarmFederate_Colab_Complete.py? (Y/n): ').strip().lower()
    write_script = write_script if write_script != '' else 'y'
    if write_script == 'y':
...
if resp == 'y':
    ...
    !python FarmFederate_Colab_Complete.py
    ...
    if RUN_ON_COLAB or os.environ.get('COLAB_HELPER_ACTIVE') == '1':
        ...
        if run_now == 'y':
            ...
            def _resolve_script_path():
                ...
                """
                expected = 'FarmFederate_Colab_Complete.py'
            ...
            try:
                ...
                print("     !python FarmFederate_Colab_Complete.py --dry-run\n", flush=True)
                print('  2) Upload `FarmFederate_Colab_Complete.py` via the Colab file upload UI or:', flush=True)
            ...
            if not locals().get('script_path'):
                print('\nAborting interactive setup: `FarmFederate_Colab_Complete.py` was not found in the session.', flush=True)
...
try:
    ...
    if not missing_labels:
        ...
        # Do NOT auto-enable synthesis. Respect explicit flags to avoid unexpected synthetic data.
        if ALLOW_SYNTHESIS:
            print('[INFO] ALLOW_SYNTHESIS=True -> synthesizing fallback datasets for missing labels.', flush=True)
        else:
            print('[ERROR] Missing real datasets for required labels and synthetic fallbacks are DISABLED.', flush=True)
            print('  To allow synthetic fallback generation, re-run with the CLI flag `--allow-synthesis` or set env var `ALLOW_SYNTHESIS=1`.', flush=True)
            print('  Alternatively, provide real datasets for the missing labels (see FALLBACK_SOURCES and KAGGLE_MAPPING) or enable `CLONE_GITHUB_REPOS=1` to attempt more fallbacks.', flush=True)
            # Write manifest and exit with non-zero code to fail fast in automated runs
            try:
                os.makedirs('results', exist_ok=True)
                with open('results/dataset_discovery_manifest.json', 'w', encoding='utf-8') as mf:
                    json.dump(DATASET_DISCOVERY_MANIFEST, mf, indent=2)
